...
(**
[env -> exp_val result =:= exp_val ea_result]
*)
...
let rec has_duplicates : string list -> bool =
  fun l ->
    match l with
    | [] -> false
    | h::t -> (List.mem h t) || has_duplicates t

let rec record_lookup : exp_val -> string -> exp_val ea_result =
  fun record id ->
    match record with
    | RecordVal([]) -> error "Proj: field does not exist"
    | RecordVal((s,v)::t) ->
      if s=id
      then return v
      else (record_lookup (RecordVal(t)) id)
