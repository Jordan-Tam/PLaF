...
let new_ref : 'a t -> 'a -> int =
  ...
  begin
    st.data.(st.size)<-v; (*! set the data at the next available pointer to be v, the argument *)
    st.size<-st.size+1;   (*! increase the size by 1 *)
    st.size-1             (*! return the location of the new data *)
  end
